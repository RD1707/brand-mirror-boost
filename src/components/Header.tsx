        ...
        <div className="container mx-auto px-4">
          <ul className="flex items-center justify-center gap-6 py-2.5 text-sm font-medium">
            {navLinks.map((link) => (
              <li key={link.label}>
                {link.highlight ? (
                  <a href="#" className="bg-primary text-primary-foreground text-xs font-bold px-3 py-1 rounded-sm uppercase tracking-wide hover:bg-primary/90 transition-colors">
                    ATÉ 30% OFF EM PRESENTES
                  </a>
                ) : (
                  <a href="#" className="text-foreground/80 hover:text-primary transition-colors">{link.label}</a>
                )}
              </li>
            ))}
          </ul>
      ...
      {mobileMenuOpen && (
        <nav className="lg:hidden border-t border-border bg-card">
          <ul className="flex flex-col px-4 py-2">
            {navLinks.map((link) => (
              <li key={link.label}>
                <a href="#" className={`block py-2.5 text-sm font-medium transition-colors border-b border-border/50 ${link.highlight ? "text-primary font-bold" : "text-foreground/80 hover:text-primary"}`}>
